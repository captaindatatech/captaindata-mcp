...
const cleanupTimer = setInterval(() => {
  ...
  if (inMemoryStore.size > MAX_STORE_SIZE) {
    ...
    authLogger.info('Removed old session tokens to prevent memory overflow', {
...
export async function storeSessionToken(
  ...
  await safeRedisOperation(
    ...
    () => {
      ...
      if (inMemoryStore.size >= MAX_STORE_SIZE) {
        authLogger.warn('In-memory session store is full, removing oldest entries', {
          storeSize: inMemoryStore.size,
        });
        const entries = Array.from(inMemoryStore.entries());
        entries.sort((a, b) => a[1].expiresAt - b[1].expiresAt);
...
export async function extractApiKey(
  ...
  if (!authHeader || typeof authHeader !== 'string') {
    ...
    const availableHeaders = Object.keys(headers).filter(
      (key) =>
    ...
    throw new Error(
      `Missing authentication: either X-API-Key header or Authorization header required. Available auth-related headers: ${availableHeaders.join(', ') || 'none'}`
    );
  }
  ...
  if (!authHeader.match(/^Bearer\s+/i)) {
    ...
    throw new Error(
      `Invalid Authorization header format: expected "Bearer <token>" but got "${headerValue}"`
    );
  }
  ...
  if (!token) {
    throw new Error(
      'Invalid Authorization header format: expected "Bearer <token>" but token is empty'
    );
  }
...
export function createAuthErrorResponse(error: Error, requestId: string) {
  if (error.message.includes('Missing authentication')) {
    return createErrorResponse(ERROR_CODES.MCP_AUTH_ERROR, error.message, requestId, {
      suggestion:
        'For MCP clients, ensure the session token is properly stored and the Authorization header is being sent',
  ...
  if (error.message.includes('Invalid or expired session token')) {
    return createErrorResponse(ERROR_CODES.SESSION_TOKEN_EXPIRED, error.message, requestId, {
      suggestion: 'Re-authenticate to get a fresh session token',
      tokenInfo: error.message.includes('...')
        ? 'Token preview shown in error'
        : 'No token preview available',
    });
  }
  ...
  return createErrorResponse(ERROR_CODES.INTERNAL_ERROR, 'Authentication error', requestId, {
